...
def baixaImagem(url):
    ...
    nome = separaFinal(url, "/")
    path = "K:/Unifesp_2021_2/Pcd/Trabalho4/python/imagens3/" + nome + "." + formato
    file = open(path, "wb")
    response = requests.get(url, headers=headers)
    file.write(response.content)
    file.close()

def separaFinal(url, separa):
    s = url
    parts = s.split(separa)
    x = len(parts)
    return parts[x-1]
...
if __name__ == "__main__":
    ...
    for i in threads:
        ...
        funct_part = ft.partial(baixaImagem)
        res = pool.map(funct_part, urls)
        pool.close()
        pool.join()

        time_exec = (time.time() - time_ini)
        print("Tempo de execucao com {} threads: {} ms\n".format(i, time_exec))
